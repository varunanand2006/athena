...
- **Google Calendar lookup + writes (Phase 20 read; Phase 23 create/update)** — `get_calendar_events(timeframe)` in `agent/main.py` calls `agent/calendar_client.py`. Returns a ≤10-event digest (title/start/end/location + `[id: ...]`) for a natural-language timeframe. Same Desktop app OAuth client can be reused for both Gmail and Calendar (scopes are requested at flow time). Mint refresh token with `scripts/calendar_oauth.py`. NOT exposed via the Rust MCP server.

- **Calendar create/update — scope reality + boundaries (Phase 23)** — `create_calendar_event` / `update_calendar_event` (foreground `CHAT_TOOLS` only, NEVER in background reflection — same discipline as `update_memory`). Scope was widened **in place** on the single `gcal-secret` from `calendar.readonly` to `calendar.events`, so the old read-only refresh token MUST be re-minted (`scripts/calendar_oauth.py`) and the Phase 21 calendar feed now shares a write-capable token by choice. **No Gmail scope grants drafts or message-labeling without also granting send** — that's why Gmail stayed read-only (the "no send" guarantee would have to live in code, not the credential). `calendar.events` likewise permits delete, but there is **no `events().delete()` call** in `calendar_client.py` — "no delete" is a code guarantee, refused in-prompt. Naive datetimes are interpreted in the primary calendar's own timezone (`calendars().get('primary').timeZone`), falling back to the `CALENDAR_TIMEZONE` env — never guess the user's tz. Reschedule is a find-then-update two-step: `get_calendar_events` (read the `[id]`) → `update_calendar_event(event_id, ...)`.
